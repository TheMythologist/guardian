...
class RollingAverage:
    def __init__(self, max_dp=100):
        ...
        if not max_dp > 0:
            raise ValueError(
                "Cannot create a rolling average of " + str(max_dp) + " data points"
    ...
    def add_value(self, value):
        ...
        if (
            self.is_full
        ):  # Simple moving average. Need to look at value at current index before overriding it.
            self.result += (value - self.storage[self.next_idx]) / (
                len(self.storage)
            )  # Get the delta div. max_dp.
    ...
    def __get_next_idx(self):
        ...
        if (
            nxt == 0
        ):  # We looped back to the start of the circular buffer, meaning we've run out of space.
...
if __name__ == "__main__":
    ...
    print(
        rl.get_avg()
    )  # should be 25.0, but is actually 24.99999999... due to floating point error.

    one = RollingAverage(1)
    print(one.add_value(999))  # 999.0
    if not one.is_full:
        raise AttributeError(
            "Rolling Average 'one' is meant to be marked as full but is not."
        )
    print(
        one.add_value(-2)
    )  # -2.0 (the average of 1 data-point is just that data-point)

    try:
        RollingAverage(
            0
        )  # A rolling average of 0 data points does not make sense. Attempting to do so
    ...
    try:
        RollingAverage(
            -7
        )  # A rolling average of a negative amount of data points also does not make sense.
