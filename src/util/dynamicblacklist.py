...
class DynamicBlacklist:
    ...
    def get_azure_ip_ranges_download(
        ...
        try:
            ...
            re_files = re.findall(
                DynamicBlacklist.MICROSOFT_DOWNLOAD_REGEX, str(response.content)
            )
            if re_files is None:
                raise ScrapeError(
            ...
            return DynamicBlacklist.determine_best_azure_file(files)
    ...
    def parse_azure_ip_ranges_from_url(url_to_json_file: str) -> list[str]:
        ...
        return DynamicBlacklist.parse_azure_ip_ranges(response.content)
    ...
    def parse_azure_ip_ranges_from_file(location_of_file: str) -> list[str]:
        with open(location_of_file, mode="rb") as file:
            return DynamicBlacklist.parse_azure_ip_ranges(file.read())
    ...
    def get_dynamic_blacklist(backup_file: str = "db.json") -> set[tuple[int, int]]:
        ...
        try:
            download_link, content = DynamicBlacklist.get_azure_ip_ranges_download()
            ranges = DynamicBlacklist.parse_azure_ip_ranges(content)
            with open(backup_file, mode="wb") as file:
                file.write(
                    DynamicBlacklist.azure_file_add_timestamp(content, download_link)
                )
            ranges.extend(DynamicBlacklist.T2_EU)  # add R* EU ranges
            ranges.extend(DynamicBlacklist.T2_US)  # add R* US ranges
            ...
            ranges = DynamicBlacklist.parse_azure_ip_ranges_from_file(backup_file)
